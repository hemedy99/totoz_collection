...
require_once "../../models/UserStatus.php";
require_once "../../models/HttpResponse.php";

$db = new Database();
$user = new UserStatus($db);
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $resultsData = isset($_GET['id']) ? $user->fetchOneUserStatus($_GET['id']) : $user->fetchAllUserStatus();

    if ($resultsData === 0) {
        $message = "No UserStatus ";
    ...
    $results = $user->insertUserStatus($userReceived);
    if ($results === -1) {
  ...
  if ($results === 0) {
    // Post NOT Found
    $http->notFound("UserStatus with the id $userReceived->id was not found");
  }else {
    // USER CAN UPDATE
    $parameters = [
      'id' => $userReceived->id,
    ...
    $resultsData = $user->updateUserStatus($parameters);
      $http->OK($resultsData);
  ...
  if ($results === 0) {
    // POST NOT FOUND
    $http->notFound("UserStatus with the id $idReceived->id was not found");
    exit();
  }
  else {
    // User CAN NOW DELETE USER
    $resultsData = $user->deleteUserStatus($idReceived->id);
